...
function renderMessages() {
    ...
    sortedMessages.forEach((msg) => {
        try {
            ...
            messageItem.setAttribute('data-message-id', msg.messageId); // Add ID for potential future use
            ...
            dataPre.className = "message-data"; // No longer contenteditable
            ...
            originBtn.textContent = "Resend to Origin"; // Changed text slightly
            const originKey = getStorageKeyForUrl(msg.origin);
            originBtn.addEventListener('click', () => sendMessageTo(originKey, originBtn)); // Keep original resend

            const destBtn = document.createElement("button");
            destBtn.className = "send-destination";
            destBtn.textContent = "Resend to Destination";
            const destKey = getStorageKeyForUrl(msg.destinationUrl);
            destBtn.addEventListener('click', () => sendMessageTo(destKey, destBtn)); // Keep original resend

            const editBtn = document.createElement("button");
            editBtn.className = "edit-send";
            editBtn.textContent = "Edit & Send";
            editBtn.addEventListener('click', () => showEditModal(msg)); // Call the new modal function

            controls.appendChild(originBtn);
            controls.appendChild(destBtn);
            controls.appendChild(editBtn); // Add the new button
...
function showEditModal(messageObject) {
    ...
    modal.className = 'edit-message-modal'; // Ensure this class is styled in dashboard.css
...
async function sendMessageFromModal(targetKey, editedDataString, buttonElement, originalButtonText) {
    ...
    buttonElement.classList.remove('success', 'error'); // Clear previous states

    try {
        const iframe = document.createElement('iframe');
        iframe.style.display = 'none';
        document.body.appendChild(iframe); // Append temporary iframe
        ...
        if (iframe.contentWindow) {
            ...
            // Cleanup iframe after successful send
        ...
        // Cleanup iframe even on error
        const tempIframe = document.body.querySelector(`iframe[src="${targetKey}"]`); // Attempt to find it again
...
function setupCallbackUrl() {
    ...
    chrome.storage.session.get([CALLBACK_URL_STORAGE_KEY], (result) => {
        ...
        if (storedUrl) {
            ...
            window.frogPostState.callbackUrl = storedUrl; // Update global state (if needed, though reading from session storage is better)
    ...
    function updateCallbackStatus(url, errorMessage = null) {
        ...
        statusElement.className = 'callback-status'; // Reset classes
        if (errorMessage) {
...
async function handlePlayButton(endpoint, button, skipCheck = false) {
    ...
    try {
        ...
        if (error.message !== "User cancelled") {
            log.error(`[Play Button Error] for ${originalFullEndpoint}:`, error.message, error.stack);
            updateButton(button, 'error');
            if (traceButton)
            ...
            log.info("User cancelled Play action.");
            if (button.textContent === '▶' || button.classList.contains('checking') || button.classList.contains('default')) {
