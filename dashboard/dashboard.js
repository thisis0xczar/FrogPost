...
async function handleTraceButton(endpoint, traceButton) {
    ...
    let handlerTracerInstance = null;
    ...
    try {
        ...
        if (typeof window.HandlerTracer !== 'function') {
            throw new Error("HandlerTracer class definition not found on window object.");
        }
        handlerTracerInstance = new window.HandlerTracer();

        if (typeof window.analyzeHandlerStatically === 'function') {
            staticAnalyzer = window.analyzeHandlerStatically;
            log.handler("Static analyzer function found on window.");
        } else {
            log.warn("Trace Warning: analyzeHandlerStatically function not found on window. Static analysis disabled.");
            staticAnalyzer = null;
        }
        ...
        log.handler(`[Trace] Effective endpoint URL: ${endpointUrlUsedForAnalysis}, Storage key: ${analysisStorageKey}`);
        ...
        log.handler(`[Trace] Using ${relevantMessages.length} messages for analysis.`);

        updatePhase('analysis');
        await new Promise(r => setTimeout(r, 50));

        let staticAnalysisResults = null;
        if (staticAnalyzer) {
            try {
                staticAnalysisResults = staticAnalyzer(handlerCode);
                if (staticAnalysisResults?.success && staticAnalysisResults.analysis) {
                    log.handler(`[Trace] Static analysis successful. Paths: ${staticAnalysisResults.analysis.accessedEventDataPaths?.size}, Flows: ${staticAnalysisResults.analysis.dataFlows?.length}`);
                } else {
                    log.warn(`[Trace] Static analysis failed or returned no/invalid analysis object: ${staticAnalysisResults?.error}.`);
                    staticAnalysisResults = null;
                }
            } catch(e) {
                log.error("Error executing static analyzer:", e);
                staticAnalysisResults = null;
            }
        } else {
            staticAnalysisResults = null;
        ...
        // Perform vulnerability analysis, passing static data if available
        vulnAnalysis = handlerTracerInstance.analyzeHandlerForVulnerabilities(
            handlerCode,
            staticAnalysisResults?.analysis // Pass nested analysis object
        );
        log.handler(`[Trace] Vuln Analysis Complete. Sinks: ${vulnAnalysis.sinks?.length}, Issues: ${vulnAnalysis.securityIssues?.length}`);


        if (relevantMessages.length > 0) {
            log.handler("[Trace] Analyzing structures from messages.");
            usedStaticAnalysis = false;
            uniqueStructures = handlerTracerInstance.analyzeJsonStructures(relevantMessages);
            log.handler(`[Trace] Found ${uniqueStructures.length} unique structures from messages.`);
        } else if (staticAnalysisResults?.success && staticAnalysisResults.analysis?.accessedEventDataPaths?.size > 0) {
            log.handler("[Trace] No messages, generating synthetic structure from static analysis.");
            usedStaticAnalysis = true;
            if (typeof createSyntheticStructureFromPaths === 'function') {
                uniqueStructures = createSyntheticStructureFromPaths(staticAnalysisResults.analysis.accessedEventDataPaths);
                log.handler(`[Trace] Generated ${uniqueStructures.length} synthetic structure(s).`);
            } else {
                log.error("[Trace] createSyntheticStructureFromPaths function not available. Cannot generate structure.");
                uniqueStructures = [];
            }
        } else {
            log.warn("[Trace] No messages and static analysis ineffective for structure generation.");
            usedStaticAnalysis = false;
            uniqueStructures = [];
        }
        ...
        log.handler("[Trace] Calling generateFuzzingPayloads with:", { uniqueStructures: uniqueStructures?.length, vulnAnalysis, relevantMessages: relevantMessages?.length });
        payloads = handlerTracerInstance.generateFuzzingPayloads(
            uniqueStructures,
            vulnAnalysis,
            relevantMessages
        );
        log.handler(`[Trace] Generated ${payloads.length} payloads.`);

        updatePhase('saving');
        const securityScore = handlerTracerInstance.calculateRiskScore(vulnAnalysis);
        ...
        report = {
            ...
            details: {
                ...
                staticAnalysisUsed: usedStaticAnalysis,
        ...
        log.info(`[Trace] Saving report and ${payloads.length} payloads using storage key: ${analysisStorageKey}`);
        ...
        await chrome.storage.local.set({
            [traceInfoKey]: {
                ...
                usedStaticAnalysis: usedStaticAnalysis
            }
        });
        log.handler(`Saved trace status for UI key ${traceInfoKey}: success=true, criticalSinks=${hasCriticalSinks}, usedStatic=${usedStaticAnalysis}...`);
        ...
        log.error(`[Trace] Error for ${originalFullEndpoint}:`, error.message);
        updateTraceButton(traceButton, 'error');
        const traceInfoKey = `trace-info-${endpointKey}`;
        try {
