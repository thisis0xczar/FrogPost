...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    try {
        ...
        const payloadIndex = message?.payloadIndex;
        ...
        switch (messageType) {
            ...
            case "postMessageCaptured":
                if (payload) {
                    ...
                    if (finalData === undefined) calculatedMessageType = "undefined";
                    else if (finalData === null) calculatedMessageType = "null";
                    else if (Array.isArray(finalData)) calculatedMessageType = "array";
                    else if (typeof finalData === 'object') {
                        calculatedMessageType = finalData.constructor === Object ? "object" : "special_object";
                    }
                    ...
                    if (newConnection) { const connectionsPayload = {}; frameConnections.forEach((v, k) => { connectionsPayload[k] = Array.from(v); }); notifyDashboard('newFrameConnection', connectionsPayload); }
                }
            ...
            case "getConsoleSuccessIndices":
                const indicesToSend = [...consoleSuccessIndices];
                consoleSuccessIndices = [];
                responseFunction({ success: true, indices: indicesToSend });
                return true;
            case "fetchInitialState":
                isAsync = true;
                (async () => { const messages = messageBuffer.getMessages(); const handlerKeys = Array.from(endpointsWithDetectedHandlers); if (responseFunction) { try { responseFunction({ success: true, messages: messages, handlerEndpointKeys: handlerKeys }); } catch (e) {} } })();
                return true;
            case "resetState":
