...
(function(global) {
    ...
    class ImprovedMessageFuzzer {
        ...
        runPayloadGeneration() {
            console.log("[Fuzzer] Starting payload generation sequence..."); this.payloads = [];
            if (!window.FuzzingPayloads) { console.error("[Fuzzer runPayloadGeneration] window.FuzzingPayloads not available."); return []; }
            const allXssPayloads = window.FuzzingPayloads.XSS || []; console.log(`[Fuzzer] Total XSS payloads available: ${allXssPayloads.length}`); const payloadList = allXssPayloads;
            for (const structure of this.messageStructures) {
                if (!structure || !structure.original) continue;
                if (structure.type === 'object') {
                    if (this.fuzzerConfig.enableSmartFuzzing && this.vulnerablePaths && this.vulnerablePaths.length > 0) { this.generateSmartObjectPayloads(structure, this.vulnerablePaths, payloadList); }
                    if (this.fuzzerConfig.enableDumbFuzzing) { this.generateDumbObjectPayloads(structure, payloadList); }
                    if (this.fuzzerConfig.enablePrototypePollution) { this.generatePrototypePollutionPayloads(structure); }
                } else if (structure.type === 'raw_string') { this.generateRawStringPayloads(structure.original, payloadList); }
            }
            ...
            console.log(`[Fuzzer] DETAILED PAYLOAD STATS:`); const typeStats = {}; this.payloads.forEach(p => { const type = p.type || 'unknown'; typeStats[type] = (typeStats[type] || 0) + 1; });
            Object.entries(typeStats).forEach(([type, count]) => { console.log(`[Fuzzer]   - ${type}: ${count} payloads`); });
    ...
    global.SinkAwarePostMessageFuzzer = class SinkAwarePostMessageFuzzer {
        ...
        generatePayloads() {
            let basePayloads = []; let ranFallbackGenerator = false;
            if (this.config?.payloads?.length > 0) { basePayloads = this.config.payloads; } else if (this.config?.traceData?.payloads?.length > 0) { basePayloads = this.config.traceData.payloads; }
            this.fuzzer.initialize(this.config?.messages || [], this.config?.handler || '', this.config?.sinks || [], this.config?.target, this.config?.callbackUrl);
            if (this.config?.fuzzerOptions) { this.fuzzer.initializeWithConfig(this.config.fuzzerOptions); }
            if (basePayloads.length === 0) { this.fuzzer.runPayloadGeneration(); basePayloads = this.fuzzer.payloads; ranFallbackGenerator = true; }
            let additionalCallbackPayloads = []; const shouldAddCallbacks = !ranFallbackGenerator && this.config?.fuzzerOptions?.enableCallbackFuzzing && this.fuzzer.callbackUrl;
            if (shouldAddCallbacks) { this.fuzzer.payloads = []; this.fuzzer.generateCallbackPayloads(); additionalCallbackPayloads = this.fuzzer.payloads; }
            const combinedPayloads = [...basePayloads, ...additionalCallbackPayloads]; const uniquePayloadStrings = new Set();
            const uniqueFinalPayloads = combinedPayloads.filter(payloadItem => { const actualPayload = payloadItem.payload !== undefined ? payloadItem.payload : payloadItem; let payloadString; try { payloadString = (typeof actualPayload === 'string') ? actualPayload : JSON.stringify(actualPayload); } catch (e) { return true; } if (uniquePayloadStrings.has(payloadString)) { return false; } else { uniquePayloadStrings.add(payloadString); return true; } });
            if (uniqueFinalPayloads.length === 0) { console.error('[SinkAware] Failed to generate or find any payloads after deduplication.'); } else { const finalTypeStats = {}; uniqueFinalPayloads.forEach(p => { const type = p.type || 'unknown'; finalTypeStats[type] = (finalTypeStats[type] || 0) + 1; }); console.log(`[SinkAware] Returning ${uniqueFinalPayloads.length} unique total payloads for execution.`); console.log("[SinkAware] Final Unique Payload Stats:"); Object.entries(finalTypeStats).forEach(([type, count]) => { console.log(`[SinkAware]    - ${type}: ${count}`); }); }
            return uniqueFinalPayloads;
        }

        executeFuzzing(payloads) {
            if (!payloads || payloads.length === 0) { console.warn('[Fuzzer Execute] No payloads passed to executeFuzzing.'); this.isExecutingPayloads = false; if (typeof this._onCompleteCallback === 'function') this._onCompleteCallback(); return; } if (this.isExecutingPayloads) return; this.isExecutingPayloads = true;
            const sentPayloads = new Set(); let messageIndex = 0; const iframe = document.getElementById('targetFrame');
            if (!iframe?.contentWindow) { console.error('[Fuzzer Execute] Target iframe not available'); this.isExecutingPayloads = false; if(typeof updateStatus === 'function') updateStatus('Error: Target iframe not found.', true); if (typeof this._onCompleteCallback === 'function') this._onCompleteCallback(); return; }
            if(typeof updateStatus === 'function') updateStatus(`Executing ${payloads.length} payloads... (0%)`); if (this.payloadIntervalId) clearInterval(this.payloadIntervalId);
            this.payloadIntervalId = setInterval(() => {
                if (!this.isExecutingPayloads || !iframe?.contentWindow || messageIndex >= payloads.length) { if(typeof updateStatus === 'function') { if(this.isExecutingPayloads) updateStatus(`Execution finished. Sent ${messageIndex} payloads.`); } console.log(`[Fuzzer Execute] Completed interval. Sent ${messageIndex} payloads. isExecuting=${this.isExecutingPayloads}`); this.stop(); return; }
                const progress = Math.round(((messageIndex + 1) / payloads.length) * 100); if(typeof updateStatus === 'function') updateStatus(`Executing payload ${messageIndex + 1}/${payloads.length} (${progress}%)`);
                const fuzzerPayload = payloads[messageIndex]; messageIndex++;
                try { const payloadId = typeof fuzzerPayload?.payload === 'string' ? `${fuzzerPayload.type || 'unknown'}-${fuzzerPayload.payload.substring(0, 30)}` : `${fuzzerPayload?.type || 'unknown'}-${messageIndex}`; if (sentPayloads.has(payloadId)) return; sentPayloads.add(payloadId); let payloadToSend = fuzzerPayload?.isRawString ? fuzzerPayload.payload : (fuzzerPayload?.payload !== undefined ? fuzzerPayload.payload : fuzzerPayload); if (payloadToSend === undefined || payloadToSend === null) { console.warn(`[Fuzzer Execute] Skipping null/undefined payload at index ${messageIndex - 1}.`); return; } const processedPayload = this.replaceJwtTokens(payloadToSend); if (typeof window.sendToFrame === 'function') window.sendToFrame(processedPayload); else iframe.contentWindow.postMessage(processedPayload, '*'); }
                catch (error) { console.error(`[Fuzzer Execute] Error sending payload index ${messageIndex - 1}:`, error, fuzzerPayload); }
            }, 200);
        }
