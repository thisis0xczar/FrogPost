...
class HandlerTracer {
    constructor() {
        ...
        this.securityChecks = [
            { name: "Missing origin check", pattern: null, severity: "High", checkFunc: (code, analysis) => analysis?.originValidationChecks?.some(c => c.strength === 'Missing') },
    ...
    identifyPathsToFuzz(structure, currentPath = '', paths = []) {
        ...
        if (structure.type !== 'object' && structure.type !== 'array') {
            if (nodePath) {
                paths.push({ path: nodePath, type: structure.type });
            }
            return paths;
        }
    ...
    analyzeHandlerForVulnerabilities(handlerCode, staticAnalysisData = null) {
        ...
        // --- Combine Regex and AST Sink Detection (No change here) ---
        this.domXssSinks.forEach(sink => {
            if (sink.methods.includes('regex')) {
                let match; const regex = new RegExp(sink.pattern.source, 'g' + (sink.pattern.flags || ''));
                while ((match = regex.exec(handlerCode)) !== null) {
                    const exactMatchSnippet = match[0]; const key = `${sink.name || sink.type}#${exactMatchSnippet}`;
                    if (!foundSinks.has(key)) { const rawContext = this.extractContext(handlerCode, match.index, exactMatchSnippet.length); let highlightedContextHTML = escapeHTML(rawContext); let highlightStartIndex = -1; let highlightEndIndex = -1; const matchIndexInRawContext = rawContext.indexOf(exactMatchSnippet); if (matchIndexInRawContext !== -1) { highlightStartIndex = matchIndexInRawContext; highlightEndIndex = highlightStartIndex + exactMatchSnippet.length; const partBefore = rawContext.substring(0, highlightStartIndex); const partMatch = rawContext.substring(highlightStartIndex, highlightEndIndex); const partAfter = rawContext.substring(highlightEndIndex); highlightedContextHTML = partBefore + '<span class="highlight-finding">' + escapeHTML(partMatch) + '</span>' + partAfter; }
        ...
        if (staticAnalysisData?.dataFlows) {
            ...
            (staticAnalysisData.dataFlows || []).forEach(flow => {
                this.domXssSinks.filter(p => p.methods.includes('ast')).forEach(sinkPattern => {
                    let isMatch = false; if (sinkPattern.nodeType && sinkPattern.nodeType !== flow.nodeType) return; if (sinkPattern.argIndex !== undefined && sinkPattern.argIndex !== flow.argIndex) return; let contextToMatch = flow.destinationContext || ""; if(sinkPattern.pattern instanceof RegExp){ const testRegex = new RegExp(sinkPattern.pattern.source, sinkPattern.pattern.flags.replace('g', '')); isMatch = testRegex.test(contextToMatch); } else if(typeof sinkPattern.pattern === 'string'){ isMatch = contextToMatch.includes(sinkPattern.pattern); }
                    if (isMatch) { const context = flow.fullCodeSnippet || flow.taintedNodeSnippet || ''; const key = `${sinkPattern.name || sinkPattern.type}#${context}`; if (!foundSinks.has(key)) foundSinks.set(key, { type: sinkPattern.name || sinkPattern.type, severity: sinkPattern.severity, path: flow.sourcePath || '(root)', conditions: flow.guardingConditions || [], context: escapeHTML(context), highlightStart: -1, highlightEnd: -1, method: 'ast', category: sinkPattern.category || 'generic' }); }
                });
            });
        }
        ...
        let originCheckCoveredByStatic = false; // Flag to prevent duplicate regex check

        if (hasListener && originChecks.length > 0) {
            originChecks.forEach(check => {
                // --- MODIFICATION: Changed severity for static 'Missing' check ---
                const severity = check.strength === 'Missing' || check.strength === 'Weak' ? 'Medium' : (check.strength === 'Medium' ? 'Medium' : 'Low'); // Missing/Weak is now Medium
                ...
                if (!existing) {
                    vulnerabilities.securityIssues.push({
                        type: issueType,
                        severity: severity,
                        context: check.snippet || `Detected origin check type: ${check.type || 'Unknown'}`,
                        details: check.type || 'N/A',
                        strength: check.strength,
                        highlightStart: -1, highlightEnd: -1
                    });
                }
            });
            // Mark that static analysis handled origin checks (even if missing)
            originCheckCoveredByStatic = true;
        }

        // Run remaining regex-based security checks
        for (const check of this.securityChecks) {
            // --- MODIFICATION: Skip regex origin check if static analysis already covered it ---
            ...
            if (check.checkFunc && staticAnalysisData) {
                ...
                let match; try { const flags = [...new Set(['g', 'm', 's', ...(check.pattern.flags?.split('') || [])])].join(''); const regex = new RegExp(check.pattern.source, flags);
                    while ((match = regex.exec(handlerCode)) !== null) {
                        const exactMatchSnippet = match[0]; const rawContext = this.extractContext(handlerCode, match.index, exactMatchSnippet.length); let highlightedContextHTML = escapeHTML(rawContext); let highlightStartIndex = -1; let highlightEndIndex = -1; const matchIndexInRawContext = rawContext.indexOf(exactMatchSnippet); if (matchIndexInRawContext !== -1) { highlightStartIndex = matchIndexInRawContext; highlightEndIndex = highlightStartIndex + exactMatchSnippet.length; const partBefore = rawContext.substring(0, highlightStartIndex); const partMatch = rawContext.substring(highlightStartIndex, highlightEndIndex); const partAfter = rawContext.substring(highlightEndIndex); highlightedContextHTML = partBefore + '<span class="highlight-finding">' + escapeHTML(partMatch) + '</span>' + partAfter; }
                        if (!vulnerabilities.securityIssues.some(iss => iss.type === check.name && iss.context.includes(escapeHTML(exactMatchSnippet)))) vulnerabilities.securityIssues.push({ type: check.name, severity: check.severity, context: highlightedContextHTML, highlightStart: highlightStartIndex, highlightEnd: highlightEndIndex });
        ...
        if (originCheckCoveredByStatic) {
            vulnerabilities.securityIssues = vulnerabilities.securityIssues.filter(issue => {
                const isStaticOriginIssue = issue.strength && issue.type.toLowerCase().includes('origin check'); // Checks added by static analysis have 'strength'
                const isOldRegexOriginIssue = !issue.strength && issue.type.toLowerCase().includes('origin check'); // Old regex checks likely lack 'strength'
                return isStaticOriginIssue || !isOldRegexOriginIssue; // Keep static results OR non-origin issues
            });
        }
    ...
    async generateFuzzingPayloads(uniqueStructures, vulnerabilities, originalMessages = []) {
        ...
        try {
            ...
            if (callbackUrl && window.FuzzingPayloads?.CALLBACK_URL) {
                processedCallbackPayloads = window.FuzzingPayloads.CALLBACK_URL.map(template =>
        ...
        structureLoop: for (const structure of uniqueStructures) {
            ...
            if (this.isPlainObject(baseMsgData)) {
                ...
                for (const sink of sinkPathsForThisStructure) {
                    ...
                    for (const payloadString of payloadsToInject) {
                        ...
                        try {
                            ...
                            if(isCallback) {
                                payloadType = isStaticStructure ? 'smart-static-callback' : 'smart-message-callback';
                            }
                            if (customPayloadsActive && !isCallback) {
                                payloadType = isStaticStructure ? 'custom-smart-static' : 'custom-smart-message';
                            }
                    ...
                    for (const field of fieldsToTarget) {
                        ...
                        for (const payloadString of selectedPayloads) {
                            ...
                            try {
                                ...
                                if(isCallback) {
                                    payloadType = 'dumb-json-callback';
                                }
                                if (customPayloadsActive && !isCallback) {
                                    payloadType = 'custom-dumb-json';
                                }
                                generatedPayloads.push({
                ...
                if(customPayloadsActive) {
                    payloadTypeBase = 'custom-dumb-string';
                }
        ...
        if (generatedPayloads.length === 0 && uniqueStructures.length === 0) {
            ...
            if (stringMessages.length > 0) {
                ...
                if(customPayloadsActive) {
                    payloadType = 'custom-dumb-generic';
                }
    ...
    calculateRiskScore(analysisResults) {
        ...
        issues.forEach(issue => {
            // --- MODIFICATION: Check for specific origin issue types/strengths ---
            if (issue.type.toLowerCase().includes('origin check')) {
                switch (issue.strength?.toLowerCase()) {
                    case 'missing': penaltyScore += 15; break; // Medium penalty
                    case 'weak': penaltyScore += 15; break;    // Medium penalty
                    case 'medium': penaltyScore += 5; break;   // Lower penalty
                    case 'strong': penaltyScore += 0; break;   // No penalty
                    default: penaltyScore += 5; break;         // Default medium-low penalty if strength unknown
                }
            } else { // Handle other security issues
        ...
        if (dataFlows.length > 0) {
            ...
            dataFlows.forEach(flow => {
                switch (flow.severity?.toLowerCase()) { // Assuming flows might have severity later
        ...
        if (issues.some(issue => issue.type.toLowerCase().includes('window.parent') && issue.type.toLowerCase().includes('origin check'))) {
            penaltyScore += 10; // Keep specific penalty for this pattern
        }
    ...
    createSyntheticStructureFromPaths(pathsSet) {
        ...
        sortedPaths.forEach(path => {
            const parts = path.match(/([^[.\]]+)|\[['"`]?([^\]'"`]+)['"`]?\]/g)?.map(part => part.startsWith('[') ? part.substring(1, part.length - 1).replace(/['"`]/g, '') : part) || [];
        ...
        if (pathsSet.has('(root)') && Object.keys(structure).length === 0) {
            return [{
                type: 'raw_value', original: getDefaultValue('(root)'), fields: [], fieldTypes: {}, pathsToFuzz: [{path: '(root)', type: 'string'}], source: 'static-analysis'
            }];
        }
        ...
        const typeFields = fields.filter(f => ['type', 'action', 'messagetype', 'kind', 'cmd'].includes(f.toLowerCase()));
        const structureVariants = [];

        if (typeFields.length > 0) {
            const commonTypeValues = ['message', 'request', 'response', 'config', 'init', 'load', 'save'];
            ...
            commonTypeValues.forEach(typeValue => {
                ...
                structureVariants.push({
                    type: 'object', original: variant, pathsToFuzz: fields.map(f => ({ path: f, type: fieldTypes[f] || 'unknown' })), examples: [{ data: JSON.parse(JSON.stringify(variant)) }], source: 'static-analysis-variant'
                });
            });
        ...
        return [{
            type: 'object', original: JSON.parse(JSON.stringify(structure)), pathsToFuzz: fields.map(f => ({ path: f, type: fieldTypes[f] || 'unknown' })), examples: [{ data: JSON.parse(JSON.stringify(structure)) }], source: 'static-analysis'
        }];
    }
    ...
    matchSinkPattern(destContext) {
        ...
        for (const sink of this.domXssSinks) {
            ...
            if (isMatch) {
                return { name: sink.name, severity: sink.severity, category: sink.category || 'generic' };
            }
        }
        return null;
    }
    ...
    async generateAstBasedPayloads(staticAnalysisData, vulnerabilityAnalysis) {
        ...
        if (dataFlows.length > 0) {
            dataFlows.forEach(flow => {
                ...
                if (flow.guardingConditions) { flow.guardingConditions.forEach(cond => { if(cond.path) accessedPaths.add(cond.path); }); }
                if (flow.taintedNodeSnippet) {
                    const matches = flow.taintedNodeSnippet.match(/event\.data\.([\w.[\]'"`]+)/g);
                    if (matches) {
                        matches.forEach(match => {
                            let path = match.replace('event.data.', '').replace(/\[['"`]?([^\]'"`]+)['"`]?\]/g, '.$1');
                            if (path) { accessedPaths.add(path); }
                        });
                    }
                }
            });
        ...
        structureLoop: for (const structure of syntheticStructures) {
            if (generatedPayloads.length >= MAX_PAYLOADS_TOTAL || !structure.type || structure.type !== 'object' || !structure.original) {
                continue;
            }
            ...
            for (const path of pathsInStructure) {
                ...
                for (const payloadString of payloadsToInject) {
                    ...
                    try {
                        ...
                        if(isCallback) {
                            currentPayloadType = payloadType.startsWith('custom-') ? (useSmartLimit ? 'custom-ast-smart-callback' : 'custom-ast-dumb-callback') : (useSmartLimit ? 'ast-smart-callback' : 'dumb-json-callback');
                        }
                        generatedPayloads.push({
    ...
    try {
        ...
        if (staticAnalyzer && handlerCode) {
            try {
                ...
                if (staticResults?.success && staticResults.analysis) {
                    ...
                    window.log.warn(`[Trace] Static analysis failed: ${staticResults?.error}.`);
                }
            } catch(e) {
        ...
        if (relevantMessages.length > 0) {
            ...
            if (staticAnalysisData && (staticAnalysisData.accessedEventDataPaths?.size > 0 || staticAnalysisData.dataFlows?.length > 0)) {
                ...
                window.log.handler(`[Trace] AST payloads: ${payloads.length}.`);
            } else {
                window.log.warn("[Trace] No data for payload gen.");
                payloads = [];
            }
        }
        ...
        const reportSaved = await window.traceReportStorage.saveTraceReport(analysisStorageKey, report);
        const payloadsSaved = await window.traceReportStorage.saveReportPayloads(analysisStorageKey, payloads);
        if (!reportSaved || !payloadsSaved) {
            throw new Error("Failed to save trace report or payloads.");
        }
        window.log.success(`Report & ${payloads.length} payloads saved: ${analysisStorageKey}`);
