...
class HandlerTracer {
    ...
    generateFuzzingPayloads(uniqueStructures, vulnerabilities, originalMessages = []) {
        ...
        if (window.FuzzingPayloads?.XSS) {
            ...
            allXssPayloads = ['<script>alert("FP_XSS")</script>', '<img src=x onerror=alert("FP_XSS")>'];
        }
        ...
        } else if (uniqueStructures.length > 0 || hasStringMessages) {
        } else {
        }
                ...
                if (useSmartFuzzing) {
                    pathLoop:
                        for (const smartPath of validSmartPaths) {
                            ...
                            const sinkPayloads = xssPayloads.slice(0, 30);
