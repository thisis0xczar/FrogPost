...
(function(global) {
    ...
    function analyzeOriginCheck(node) {
        ...
        if (node.type === 'BinaryExpression' && ['===', '!==', '==', '!='].includes(node.operator)) {
            ...
            if (['includes', 'startsWith', 'endsWith', 'indexOf', 'test', 'match'].includes(method)) {
                ...
                if (isEventOriginAccess(targetObject)) { // event.origin.includes(...)
                    originNode = targetObject; comparisonValue = argumentValue; checkType = `Method Call (.${method})`; strength = 'Weak';
                } else if (argumentValue && isEventOriginAccess(argumentValue) && targetObject.type === 'Identifier') { // allowedOrigins.includes(event.origin)
                    originNode = argumentValue; comparisonValue = targetObject; checkType = `Lookup (.${method} on ${targetObject.name})`; strength = 'Medium';
                } else if (argumentValue && isEventOriginAccess(argumentValue) && method === 'test' && targetObject.type === 'Literal' && targetObject.regex) { // /regex/.test(event.origin)
    ...
    function analyzeConditionNode(node) { // Keep original condition analyzer for data checks
        if (!node) return null;
    ...
    function extractGuardingConditions(ancestors) { // This extracts data conditions
        const conditions = [];
    ...
    global.analyzeHandlerStatically = function(handlerCode) {
            ...
            global.acorn.walk.ancestor(ast, {
                MemberExpression(node, ancestors) {
                    ...
                    if (isEventOriginAccess(node)) {
                        analysisResults.hasListener = true; // Accessing origin implies it's likely a listener
                ...
                IfStatement(node, ancestors) {
                    analysisResults.hasListener = true; // If implies logic, likely listener
                ...
                CallExpression(node, ancestors) {
                    analysisResults.hasListener = true; // Calls imply activity
                    const originCheckResult = analyzeOriginCheck(node); // Check if the call itself is an origin check method
                    if (originCheckResult) {
                        analysisResults.originValidationChecks.push(originCheckResult);
                        foundOriginCheck = true;
                    }
                    // Existing CallExpression logic for data flow
                ...
                AssignmentExpression(node, ancestors) {
                    analysisResults.hasListener = true; // Assignments imply activity
                ...
                NewExpression(node, ancestors) { // Keep existing NewExpression logic
                    node.arguments.forEach((argNode, index) => {
                        const eventDataSourceNode = findEventDataSourceNode(argNode);
                        if (eventDataSourceNode) {
                            const fullSourcePath = getFullAccessPath(eventDataSourceNode);
        ...
        // Add 'Missing' check if applicable
        ...
        // Calculate requiredConditions based on data flow conditions
