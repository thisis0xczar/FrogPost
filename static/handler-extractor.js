...
class HandlerExtractor {
    ...
    analyzeAst(ast, scriptContent, sourceUrl) {
        const foundHandlers = []; if (!ast || typeof acorn === 'undefined' || typeof acorn.walk === 'undefined') return foundHandlers;
        try {
            acorn.walk.simple(ast, {
                AssignmentExpression: (node) => { if (node.operator === '=' && node.left.type === 'MemberExpression' && node.left.property.name === 'onmessage') { let funcNode = null; let category = 'ast-onmessage-assignment'; let functionName = null; if (node.right.type === 'FunctionExpression' || node.right.type === 'ArrowFunctionExpression') funcNode = node.right; else if (node.right.type === 'Identifier') { functionName = node.right.name; let funcDef = this.functionDefinitions.get(functionName); funcNode = funcDef?.node || null; if(funcNode) category += '-identifier'; } if (funcNode) { const codePreview = scriptContent.substring(funcNode.start, Math.min(funcNode.end, funcNode.start + 100)); if(typeof log !== 'undefined') log.debug(`[Extractor AST Found] Category: ${category}, Source: ${sourceUrl}, FuncName: ${functionName || 'N/A'}, Preview: ${codePreview}...`); foundHandlers.push({ category, source: sourceUrl, functionName, handlerNode: funcNode, fullScriptContent: scriptContent }); } } },
                CallExpression: (node) => { if (node.callee.type === 'MemberExpression' && node.callee.property.name === 'addEventListener' && node.arguments.length >= 2 && node.arguments[0].type === 'Literal' && node.arguments[0].value === 'message') { const handlerArg = node.arguments[1]; let funcDef = null; let category = 'ast-event-listener'; let functionName = null; let resolvedIdentifier = false; if (handlerArg.type === 'FunctionExpression' || handlerArg.type === 'ArrowFunctionExpression') { funcDef = { node: handlerArg }; } else if (handlerArg.type === 'Identifier') { functionName = handlerArg.name; funcDef = this.functionDefinitions.get(functionName); if (funcDef) { category += '-identifier'; resolvedIdentifier = true; } } else if (handlerArg.type === 'MemberExpression') { const objName = handlerArg.object?.name; const methodName = handlerArg.property?.name; if (methodName) { functionName = methodName; const protoKey = Array.from(this.functionDefinitions.keys()).find(key => key.endsWith(`.${methodName}`)); if (protoKey) { funcDef = this.functionDefinitions.get(protoKey); category += '-prototype-lookup'; resolvedIdentifier = true; if(typeof log !== 'undefined') log.debug(`[Extractor AST] Found prototype method ref ${protoKey} for addEventListener via MemberExpression.`); } else { const objMethodKey = `${objName}.${methodName}`; if (this.functionDefinitions.has(objMethodKey)) { funcDef = this.functionDefinitions.get(objMethodKey); category += '-objectMethod-lookup'; resolvedIdentifier = true; if(typeof log !== 'undefined') log.debug(`[Extractor AST] Found object method ref ${objMethodKey} for addEventListener via MemberExpression.`); } } } if(!resolvedIdentifier && handlerArg.object?.type === 'ThisExpression' && handlerArg.property?.name) { functionName = handlerArg.property.name; const thisProtoKey = Array.from(this.functionDefinitions.keys()).find(key => key.endsWith(`.${functionName}`) && this.functionDefinitions.get(key)?.type === 'prototype'); if(thisProtoKey) { funcDef = this.functionDefinitions.get(thisProtoKey); category += '-this-prototype-lookup'; resolvedIdentifier = true; if(typeof log !== 'undefined') log.debug(`[Extractor AST] Found this.prototype method ref ${thisProtoKey} for addEventListener.`); } } } else if (handlerArg.type === 'CallExpression' && handlerArg.callee.type === 'MemberExpression' && handlerArg.callee.property.name === 'bind') { let boundFuncIdentifier = null; let boundFuncNode = null; if (handlerArg.callee.object.type === 'Identifier') { boundFuncIdentifier = handlerArg.callee.object.name; boundFuncNode = this.functionDefinitions.get(boundFuncIdentifier); } else if (handlerArg.callee.object.type === 'MemberExpression' && handlerArg.callee.object.property?.name) { const objName = handlerArg.callee.object.object?.name; const methodName = handlerArg.callee.object.property.name; boundFuncIdentifier = methodName; const protoKey = Array.from(this.functionDefinitions.keys()).find(key => key.endsWith(`.${methodName}`) && (!objName || key.startsWith(objName))); if (protoKey) { boundFuncNode = this.functionDefinitions.get(protoKey); category += '-bind-prototype'; if(typeof log !== 'undefined') log.debug(`[Extractor AST] Found bound prototype method ref ${protoKey} for addEventListener.`); } else { const objMethodKey = `${objName}.${methodName}`; if(this.functionDefinitions.has(objMethodKey)) { boundFuncNode = this.functionDefinitions.get(objMethodKey); category += '-bind-objectMethod'; if(typeof log !== 'undefined') log.debug(`[Extractor AST] Found bound object method ref ${objMethodKey} for addEventListener.`); } } } else if (handlerArg.callee.object.type === 'FunctionExpression') { boundFuncNode = { node: handlerArg.callee.object }; category += '-bind-inline'; } if (boundFuncNode) { funcDef = boundFuncNode; functionName = boundFuncIdentifier || funcDef.methodName; resolvedIdentifier = true; category += boundFuncIdentifier ? '-identifier' : ''; } } if (funcDef && funcDef.node) { const codePreview = scriptContent.substring(funcDef.node.start, Math.min(funcDef.node.end, funcDef.node.start + 100)); if(typeof log !== 'undefined') log.debug(`[Extractor AST Found] Category: ${category}, Source: ${sourceUrl}, FuncName: ${functionName || 'N/A'}, Preview: ${codePreview}...`); foundHandlers.push({ category, source: sourceUrl, functionName: functionName || funcDef.methodName, handlerNode: funcDef.node, fullScriptContent: scriptContent }); } else if (handlerArg.type === 'Identifier' && !resolvedIdentifier) { if(typeof log !== 'undefined') log.debug(`[Extractor AST] Could not resolve identifier "${handlerArg.name}" used in addEventListener.`); } } }
            });
        } catch (e) { if(typeof log !== 'undefined') log.error(`[Extractor] Error walking AST for ${sourceUrl}:`, e); }
    ...
    analyzeInlineHandlers(doc) {
        const handlers = []; const elements = doc.querySelectorAll('[onmessage]');
    ...
    scoreHandler(handlerInfo) {
        ...
        let score = 0; const MIN_CODE_LENGTH_ESTIMATE = 40; const MAX_CODE_LENGTH_ESTIMATE = 15000; let handlerCodeLength = 0;
        if (handlerNode?.end && handlerNode?.start) handlerCodeLength = handlerNode.end - handlerNode.start; else if (handlerCode) handlerCodeLength = handlerCode.length;
        if (handlerCodeLength < MIN_CODE_LENGTH_ESTIMATE) return 0;
        if(typeof log !== 'undefined') { log.debug(`[Scoring Handler] Category: ${category}, Source: ${source?.substring(0, 100)}`); log.debug(`[Scoring Handler] Using Context - Keys:`, Array.from(this.messageKeys)); log.debug(`[Scoring Handler] Using Context - Types/Kinds:`, Array.from(this.messageTypes)); }
        let basicScore = 10; score += basicScore; let astScore = 0;
        if (handlerNode && typeof acorn !== 'undefined' && typeof acorn.walk !== 'undefined') {
            try {
                const foundGenericKeys = new Set(); const foundSpecificKeys = new Set(); const foundSpecificTypes = new Set(); let usesPostMessageCall = false; let referencesEventData = false; let referencesEventOrigin = false; let hasOriginCheck = false; let usesJsonParse = false; let usesSwitchOnKind = false; let eventParamName = null; if (handlerNode.params?.[0]?.type === 'Identifier') eventParamName = handlerNode.params[0].name;
                acorn.walk.simple(handlerNode, {
                    MemberExpression: (node) => { let propName = null; if (node.property?.type === 'Identifier') propName = node.property.name; else if (node.property?.type === 'Literal') propName = node.property.value; let baseObjectName = null; if (node.object?.type === 'Identifier') baseObjectName = node.object.name; if (propName) { if (eventParamName && baseObjectName === eventParamName && ['data', 'origin', 'source'].includes(propName)) { foundGenericKeys.add(propName); if (propName === 'data') referencesEventData = true; if (propName === 'origin') referencesEventOrigin = true; } if (eventParamName && node.object?.type === 'MemberExpression' && node.object.object?.name === eventParamName && node.object.property?.name === 'data') { if (this.messageKeys.has(propName)) { if(typeof log !== 'undefined') log.debug(`[Scoring AST Walk] Found Specific Key: ${propName}`); foundSpecificKeys.add(propName); } } if(eventParamName && propName === 'rawData' && node.object?.type === 'MemberExpression' && node.object.property?.name === 'data' && node.object.object?.type === 'MemberExpression' && node.object.object.property?.name === 'data' && node.object.object.object?.name === eventParamName){ if(this.messageKeys.has(propName)) foundSpecificKeys.add(propName); } if (propName === 'postMessage') usesPostMessageCall = true; if (propName === 'origin' && node.parent?.type === 'BinaryExpression' && ['===', '!==', '==', '!='].includes(node.parent.operator)) { hasOriginCheck = true; } } },
                    Literal: (node) => { if (typeof node.value === 'string') { if (this.messageTypes.has(node.value)) { if(typeof log !== 'undefined') log.debug(`[Scoring AST Walk] Matched Specific Type/Kind: "${node.value}"`); foundSpecificTypes.add(node.value); } } },
                ...
                astScore += foundSpecificKeys.size * 150; astScore += foundSpecificTypes.size * 100; if (referencesEventData) astScore += 20; if (referencesEventOrigin) astScore += 20; if (usesPostMessageCall) astScore += 10; if (hasOriginCheck) astScore += 75; if (usesJsonParse) astScore += 40; if (usesSwitchOnKind) astScore += 50; if (handlerCodeLength > MAX_CODE_LENGTH_ESTIMATE && astScore < 200) astScore -= 100; else if (handlerCodeLength > MAX_CODE_LENGTH_ESTIMATE) astScore -= 20;
                if(typeof log !== 'undefined') log.debug(`[Scoring AST Node SUCCEEDED] Handler (Cat: ${category}, EstLen: ${handlerCodeLength}) - AST Score: ${astScore}, Specific Keys Found: ${foundSpecificKeys.size}, Patterns: OriginCheck=${hasOriginCheck}, JSONParse=${usesJsonParse}, Switch=${usesSwitchOnKind}`);
            } catch (e) { if(typeof log !== 'undefined') log.warn(`[Scoring AST Node] Failed walking node for category ${category}: ${e.message}.`); astScore = 0; }
        } else if (handlerCode) { if (handlerCode.includes('.data') || handlerCode.includes('["data"]')) astScore += 5; if (handlerCode.includes('.origin') || handlerCode.includes('["origin"]')) astScore += 5; if (handlerCode.includes('.source') || handlerCode.includes('["source"]')) astScore += 3; if (handlerCode.includes('postMessage')) astScore += 3; if (handlerCode.includes('messageType') || handlerCode.includes('["messageType"]')) astScore += 10; if (handlerCode.includes('JSON.parse')) astScore += 5; if (handlerCode.includes('switch')) astScore += 5; if(typeof log !== 'undefined') log.debug(`[Scoring Basic String] Handler (Cat: ${category}, Len: ${handlerCodeLength}) - Basic Score Contribution: ${astScore}`); }
        score += astScore;
        if (category?.includes('runtime')) score += 150; else if (category?.includes('ast-event-listener') || category?.includes('ast-onmessage')) score += 50; else if (category?.includes('debugger')) score += 5; else if (category?.includes('inline-onmessage-attribute')) score += 5; else if (category?.includes('regex')) score += 1;
        return Math.max(0, score);
    }

    getBestHandler(handlersInfo) {
        if (!handlersInfo || handlersInfo.length === 0) return null;
        const scoredHandlers = handlersInfo.map(handlerInfo => { const score = this.scoreHandler(handlerInfo); let boostedScore = score; if (handlerInfo.category?.includes('prototype') || handlerInfo.category?.includes('objectMethod') || handlerInfo.category?.includes('indirect')) { boostedScore += 10; } else if (handlerInfo.category?.includes('ast-event-listener-identifier') || handlerInfo.category?.includes('ast-onmessage-assignment-identifier')) { boostedScore += 5; } return { ...handlerInfo, score: boostedScore }; }).filter(h => h.score > 0);
        if (scoredHandlers.length === 0) { if(typeof log !== 'undefined') log.debug("[getBestHandler] No candidates scored above 0."); return null; }
        if(typeof log !== 'undefined') log.debug("[getBestHandler] Scored Candidates (Pre-sort):", JSON.stringify(scoredHandlers.map(h => ({ score: h.score, category: h.category, source: h.source?.substring(0,100), hasNode: !!h.handlerNode, name: h.functionName || 'N/A' })), null, 2));
        const categoryPriority = { 'runtime': 1, 'ast-event-listener': 2, 'ast-onmessage': 3, 'debugger': 4, 'regex': 6, 'inline-onmessage-attribute': 7 };
        scoredHandlers.sort((a, b) => { if (b.score !== a.score) return b.score - a.score; const priorityA = categoryPriority[a.category?.split('-')[0]] || 99; const priorityB = categoryPriority[b.category?.split('-')[0]] || 99; if (priorityA !== priorityB) return priorityA - priorityB; const lenA = a.handlerNode ? a.handlerNode.end - a.handlerNode.start : (a.handler?.length || a.fullScriptContent?.length || 0); const lenB = b.handlerNode ? b.handlerNode.end - b.handlerNode.start : (b.handler?.length || b.fullScriptContent?.length || 0); if (lenA !== lenB) return lenB - lenA; return (a.source || '').localeCompare(b.source || ''); });
        const bestHandlerInfo = scoredHandlers[0]; let finalHandlerCode = bestHandlerInfo.handler || '';
        if (!finalHandlerCode && bestHandlerInfo.handlerNode && bestHandlerInfo.fullScriptContent) { try { finalHandlerCode = bestHandlerInfo.fullScriptContent.substring(bestHandlerInfo.handlerNode.start, bestHandlerInfo.handlerNode.end); } catch (e) { if(typeof log !== 'undefined') log.error("Failed to extract final handler code string from node!", e); finalHandlerCode = "[Error extracting code string]"; } } else if (!finalHandlerCode && bestHandlerInfo.fullScriptContent) { finalHandlerCode = bestHandlerInfo.fullScriptContent; }
        ...
        if(typeof log !== 'undefined') log.debug(`[getBestHandler] Selected Handler: Score=${bestHandlerInfo.score}, Category=${bestHandlerInfo.category}, Source=${bestHandlerInfo.source}, EstLen=${bestLen}, Name=${bestHandlerInfo.functionName || 'N/A'}`);
        return { handler: finalHandlerCode, category: bestHandlerInfo.category, score: bestHandlerInfo.score, source: bestHandlerInfo.source, functionName: bestHandlerInfo.functionName, handlerNode: bestHandlerInfo.handlerNode };
    }
